...
export default function OperationsChallan() {
  ...
  const load = useCallback(async () => {
    ...
    setLoading(true);
    try {
      const [cRes, rRes] = await Promise.all([authFetch(`${API_BASE}/operations/challans`), authFetch(`${API_BASE}/operations/riders`)]);
  ...
  const slotOptions = useMemo(() => [...new Set(challans.map((c) => String(c.slot || '').trim()).filter(Boolean))].sort((a, b) => a.localeCompare(b)), [challans]);
  const displayRows = useMemo(() => {
    const q = search.trim().toLowerCase();
    return challans.filter((c) => {
      if (filterDay && String(c.day || '').trim() !== filterDay) return false;
      if (filterSlot && String(c.slot || '').trim() !== filterSlot) return false;
      if (!q) return true;
  ...
  const patchRider = async (challanId, rider_id) => {
    setSaving(true);
    try {
      ...
      <div className="om-root" style={{ padding: '19px', fontFamily: "'Poppins','Inter',sans-serif", display: 'flex', flexDirection: 'column', minHeight: 0, height: '100%', overflow: 'hidden', boxSizing: 'border-box' }}>
        <div className="om-header" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px', flexWrap: 'wrap', gap: '12px', flexShrink: 0 }}>
          <div>
            ...
            <p style={{ margin: '6px 0 0', fontSize: '11px', color: '#888' }}>Print PDF respects active filters; if rows are selected then only selected rows are exported.</p>
          </div>
        ...
        <div className="om-filter-desktop" style={{ display: 'flex', flexWrap: 'wrap', gap: '10px', marginBottom: '16px', alignItems: 'flex-end' }}>
          <div style={{ flex: '1 1 220px' }}><label style={{ display: 'block', fontSize: '10px', color: '#666', marginBottom: '3px' }}>Search</label><input type="text" value={search} onChange={(e) => setSearch(e.target.value)} style={{ width: '100%', padding: '6px 10px', borderRadius: '6px', border: '1px solid #e0e0e0', fontSize: '11px' }} /></div>
          <div style={{ width: 110 }}><label style={{ display: 'block', fontSize: '10px', color: '#666', marginBottom: '3px' }}>Day</label><select value={filterDay} onChange={(e) => setFilterDay(e.target.value)} style={{ width: '100%', padding: '6px 10px', borderRadius: '6px', border: '1px solid #e0e0e0', fontSize: '11px' }}><option value="">All</option>{dayOptions.map((d) => <option key={d} value={d}>{d}</option>)}</select></div>
          <div style={{ width: 110 }}><label style={{ display: 'block', fontSize: '10px', color: '#666', marginBottom: '3px' }}>Slot</label><select value={filterSlot} onChange={(e) => setFilterSlot(e.target.value)} style={{ width: '100%', padding: '6px 10px', borderRadius: '6px', border: '1px solid #e0e0e0', fontSize: '11px' }}><option value="">All</option>{slotOptions.map((s) => <option key={s} value={s}>{s}</option>)}</select></div>
          <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap' }}>
            ...
            {emailOk && <button type="button" onClick={() => setConfirmRegen(true)} style={{ padding: '6px 13px', height: '29px', background: '#FFEBEE', color: '#C62828', border: '1px solid #FFCDD2', borderRadius: '6px', fontSize: '11px' }}>Generate data</button>}
          </div>
        </div>
        ...
        <div className="om-table-wrap" style={{ flex: 1, minHeight: '304px', overflow: 'auto', border: '1px solid #e0e0e0', borderRadius: '6px', background: '#fff' }}>
          {loading ? <div style={{ padding: '32px', textAlign: 'center', color: '#666', fontSize: '11px' }}>Loading…</div> : (
            <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '10px' }}>
              <thead><tr style={{ background: '#f5f5f5' }}><th style={{ padding: '10px 8px', borderBottom: '2px solid #e0e0e0' }}><input type="checkbox" checked={allFilteredSelected} onChange={toggleSelectAll} /></th>{['Address', 'Day', 'Slot', 'Area', 'Hissa', 'Shareholders', 'Status', 'Rider'].map((h) => <th key={h} style={{ padding: '10px 8px', textAlign: 'left', borderBottom: '2px solid #e0e0e0' }}>{h}</th>)}</tr></thead>
              <tbody>
                {displayRows.length === 0 ? <tr><td colSpan={9} style={{ padding: '24px', textAlign: 'center', color: '#666' }}>No rows found.</td></tr> : displayRows.map((c) => {
                  ...
                  return (
                    <tr key={c.challan_id} style={{ borderBottom: '1px solid #f0f0f0', cursor: c.qr_token ? 'pointer' : 'default' }} onClick={() => c.qr_token && openChallanModal(c.qr_token)}>
                      ...
                      <td style={{ padding: '10px 8px', maxWidth: 220 }}>{c.address || '—'}</td><td style={{ padding: '10px 8px' }}>{c.day || '—'}</td><td style={{ padding: '10px 8px' }}>{c.slot || '—'}</td><td style={{ padding: '10px 8px' }}>{c.area || '—'}</td><td style={{ padding: '10px 8px' }}>{c.total_hissa ?? c.order_count ?? '—'}</td>
                      ...
                      <td style={{ padding: '10px 8px' }} onClick={(e) => e.stopPropagation()}><select value={c.rider_id ?? ''} onChange={(e) => patchRider(c.challan_id, e.target.value)} style={selectStyle}><option value="">—</option>{riders.map((r) => <option key={r.rider_id} value={r.rider_id}>{r.rider_name}</option>)}</select></td>
                    </tr>
                  );
                })}
              </tbody>
