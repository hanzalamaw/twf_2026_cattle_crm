          ...
          <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '10px', tableLayout: 'auto' }}>
            <thead>
              <tr style={{ background: '#f5f5f5' }}>
                <th style={{ padding: '8px 6px', textAlign: 'left', fontWeight: '600', color: '#333', borderBottom: '2px solid #e0e0e0', whiteSpace: 'nowrap', width: '32px', fontSize: '10px' }}>
                  <input type="checkbox" checked={orders.length > 0 && selectedIds.size === orders.length} onChange={toggleSelectAll} style={{ cursor: 'pointer' }} />
                </th>
                {COLUMNS.map((col) => (
                  <th key={col.key} style={{ padding: '8px 6px', textAlign: 'left', fontWeight: '600', color: '#333', borderBottom: '2px solid #e0e0e0', whiteSpace: 'nowrap', fontSize: '10px' }}>{col.label}</th>
                ))}
                <th style={{ padding: '8px 6px', textAlign: 'left', fontWeight: '600', color: '#333', borderBottom: '2px solid #e0e0e0', whiteSpace: 'nowrap', fontSize: '10px' }}>Actions</th>
              </tr>
            </thead>
            <tbody>
              {orders.length === 0 ? (
                <tr>
                  <td colSpan={COLUMNS.length + 2} style={{ padding: '19px', textAlign: 'center', color: '#666', fontSize: '11px' }}>No orders found.</td>
                </tr>
              ) : (
                orders.map((row) => (
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '10px 16px' }}>
              ...
              <div style={{ minWidth: 0, gridColumn: '1 / -1' }}>
                ...
                <textarea
                  ...
                  rows={2}
                  style={{ width: '100%', boxSizing: 'border-box', padding: '6px 10px', borderRadius: '6px', border: '1px solid #e0e0e0', fontSize: '10px', resize: 'vertical' }}
                />
              </div>
            </div>
